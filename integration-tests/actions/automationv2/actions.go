...
func (a *AutomationTest) CollectNodeDetails() error {
	...
	if a.IsOnk8s {
		for _, node := range a.PluginNodesk8s[:] {
			nodes = append(nodes, node.PluginClient)
		}
		a.PluginNodes = nodes
	} else {
		nodes = a.PluginNodes[:]
	...
	if a.IsOnk8s {
		a.DefaultP2Pv2Bootstrapper = fmt.Sprintf("%s@%s-node-1:%d", a.NodeDetails[0].P2PId, a.PluginNodesk8s[0].Name(), 6690)
	} else {
		a.DefaultP2Pv2Bootstrapper = fmt.Sprintf("%s@%s:%d", a.NodeDetails[0].P2PId, a.PluginNodes[0].InternalIP(), 6690)
	}
	return nil
}
...
func (a *AutomationTest) AddBootstrapJob() error {
	...
	_, err := a.PluginNodes[0].MustCreateJob(bootstrapSpec)
	if err != nil {
...
func (a *AutomationTest) AddAutomationJobs() error {
	...
	for i := 1; i < len(a.PluginNodes); i++ {
		...
		_, err := a.PluginNodes[i].MustCreateJob(&autoOCR2JobSpec)
		if err != nil {
...
func (a *AutomationTest) SetConfigOnRegistry() error {
	...
	for i, donNode := range donNodes {
		index, pluginNode := i, donNode
		eg.Go(func() error {
			offchainPkBytes, err := hex.DecodeString(strings.TrimPrefix(pluginNode.OCR2OffchainPublicKey, "ocr2off_evm_"))
			if err != nil {
				return err
			}
			...
			configPkBytes, err := hex.DecodeString(strings.TrimPrefix(pluginNode.OCR2ConfigPublicKey, "ocr2cfg_evm_"))
			if err != nil {
				return err
			}
			...
			onchainPkBytes, err := hex.DecodeString(strings.TrimPrefix(pluginNode.OCR2OnChainPublicKey, "ocr2on_evm_"))
			if err != nil {
				return err
			}
			...
			oracleIdentities[index] = confighelper.OracleIdentityExtra{
				OracleIdentity: confighelper.OracleIdentity{
					...
					PeerID:            pluginNode.P2PId,
					TransmitAccount:   types.Account(pluginNode.TransmitterAddresses[a.TransmitterKeyIndex]),
				},
