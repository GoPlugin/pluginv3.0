...
// BuildMedianOCR2Config builds a default OCRv2 config for the given plugin nodes for a standard median aggregation job
...
// GetOracleIdentities retrieves all plugin nodes' OCR2 config identities with defaul key index
func GetOracleIdentities(pluginNodes []*client.PluginK8sClient) ([]int, []confighelper.OracleIdentityExtra, error) {
	return GetOracleIdentitiesWithKeyIndex(pluginNodes, 0)
}

// GetOracleIdentitiesWithKeyIndex retrieves all plugin nodes' OCR2 config identities by key index
func GetOracleIdentitiesWithKeyIndex(
	pluginNodes []*client.PluginK8sClient,
	keyIndex int,
) ([]int, []confighelper.OracleIdentityExtra, error) {
	S := make([]int, len(pluginNodes))
	oracleIdentities := make([]confighelper.OracleIdentityExtra, len(pluginNodes))
	sharedSecretEncryptionPublicKeys := make([]types.ConfigEncryptionPublicKey, len(pluginNodes))
	eg := &errgroup.Group{}
	for i, cl := range pluginNodes {
		index, pluginNode := i, cl
		eg.Go(func() error {
			addresses, err := pluginNode.EthAddresses()
			if err != nil {
				return err
			}
			ocr2Keys, err := pluginNode.MustReadOCR2Keys()
			if err != nil {
				return err
			}
			...
			keys, err := pluginNode.MustReadP2PKeys()
			if err != nil {
				return err
			}
...
func CreateOCRv2Jobs(
	...
	workerPluginNodes []*client.PluginK8sClient,
	...
	for _, pluginNode := range workerPluginNodes {
		err = pluginNode.MustCreateBridge(juelsBridge)
		if err != nil {
	...
	for _, ocrInstance := range ocrInstances {
		...
		for _, pluginNode := range workerPluginNodes {
			nodeTransmitterAddress, err := pluginNode.PrimaryEthAddress()
			if err != nil {
				return fmt.Errorf("getting primary ETH address from OCR node have failed: %w", err)
			}
			nodeOCRKeys, err := pluginNode.MustReadOCR2Keys()
			if err != nil {
				return fmt.Errorf("getting OCR keys from OCR node have failed: %w", err)
			}
			nodeOCRKeyId := nodeOCRKeys.Data[0].ID

			nodeContractPairID, err := BuildOCR2NodeContractPairID(pluginNode, ocrInstance)
			if err != nil {
				return err
			}
			...
			err = pluginNode.MustCreateBridge(bta)
			if err != nil {
			...
			_, err = pluginNode.MustCreateJob(ocrSpec)
			if err != nil {
...
// SetOCR2AdapterResponse sets a single adapter response that correlates with an ocr contract and a plugin node
// used for OCR2 tests
func SetOCR2AdapterResponse(
	response int,
	ocrInstance contracts.OffchainAggregatorV2,
	pluginNode *client.PluginK8sClient,
	mockserver *ctfClient.MockserverClient,
) error {
	nodeContractPairID, err := BuildOCR2NodeContractPairID(pluginNode, ocrInstance)
	if err != nil {
...
// SetOCR2AllAdapterResponsesToTheSameValue sets the mock responses in mockserver that are read by plugin nodes
...
// SetOCR2AllAdapterResponsesToDifferentValues sets the mock responses in mockserver that are read by plugin nodes
...
func SetOCR2AllAdapterResponsesToDifferentValues(
	...
	if len(responses) != len(ocrInstances)*len(pluginNodes) {
		return fmt.Errorf(
			"amount of responses %d should be equal to the amount of OCR instances %d times the amount of Plugin nodes %d",
			len(responses), len(ocrInstances), len(pluginNodes),
		)
	}
	eg := &errgroup.Group{}
	for _, o := range ocrInstances {
		ocrInstance := o
		for ni := 1; ni < len(pluginNodes); ni++ {
			nodeIndex := ni
			eg.Go(func() error {
				return SetOCR2AdapterResponse(responses[nodeIndex-1], ocrInstance, pluginNodes[nodeIndex], mockserver)
			})
		}
	}
	return eg.Wait()
...
// BuildOCR2NodeContractPairID builds a UUID based on a related pair of a Plugin node and OCRv2 contract
func BuildOCR2NodeContractPairID(node *client.PluginK8sClient, ocrInstance contracts.OffchainAggregatorV2) (string, error) {
	if node == nil {
