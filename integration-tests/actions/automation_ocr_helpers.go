...
func BuildAutoOCR2ConfigVars(
	...
	return BuildAutoOCR2ConfigVarsWithKeyIndex(t, pluginNodes, registryConfig, registrar, deltaStage, 0, common.Address{})
}
...
func BuildAutoOCR2ConfigVarsWithKeyIndex(
	...
	S, oracleIdentities, err := GetOracleIdentitiesWithKeyIndex(pluginNodes, keyIndex)
	if err != nil {
...
func CreateOCRKeeperJobs(
	...
	// TODO: Use service name returned by plugin-env once that is available
	P2Pv2Bootstrapper := fmt.Sprintf("%s@%s-node-1:%d", bootstrapP2PId, bootstrapNode.Name(), 6690)

	for nodeIndex := 1; nodeIndex < len(pluginNodes); nodeIndex++ {
		nodeTransmitterAddress, err := pluginNodes[nodeIndex].EthAddresses()
		require.NoError(t, err, "Shouldn't fail getting primary ETH address from OCR node %d", nodeIndex+1)
		nodeOCRKeys, err := pluginNodes[nodeIndex].MustReadOCR2Keys()
		...
		_, err = pluginNodes[nodeIndex].MustCreateJob(&autoOCR2JobSpec)
...
func deployRegistry(
	...
	ef, err := contractDeployer.DeployMockETHPLIFeed(big.NewInt(2e18))
