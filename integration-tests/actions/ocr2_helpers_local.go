...
func CreateOCRv2JobsLocal(
	...
	workerPluginNodes []*client.PluginClient,
	mockAdapter *test_env.Killgrave,
	mockAdapterPath string, // Path on the mock server for the Plugin nodes to query
...
func BuildMedianOCR2ConfigLocal(workerNodes []*client.PluginClient, ocrOffchainOptions contracts.OffchainOptions) (*contracts.OCRv2Config, error) {
...
func DeleteJobs(nodes []*client.PluginClient) error {
	for _, node := range nodes {
		if node == nil {
			return fmt.Errorf("found a nil plugin node in the list of plugin nodes while tearing down: %v", nodes)
		}
		jobs, _, err := node.ReadJobs()
		if err != nil {
			return fmt.Errorf("error reading jobs from plugin node: %w", err)
		}
		for _, maps := range jobs.Data {
			if _, ok := maps["id"]; !ok {
				return fmt.Errorf("error reading job id from plugin node's jobs %+v", jobs.Data)
			}
			id := maps["id"].(string)
			_, err2 := node.DeleteJob(id)
			if err2 != nil {
				return fmt.Errorf("error deleting job from plugin node: %w", err)
			}
		}
	}
	return nil
...
func DeleteBridges(nodes []*client.PluginClient) error {
	for _, node := range nodes {
		if node == nil {
			return fmt.Errorf("found a nil plugin node in the list of plugin nodes while tearing down: %v", nodes)
		}
