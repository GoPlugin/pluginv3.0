...
import (
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/functions/generated/functions_router"
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/operator_wrapper"
...
func (f *EthereumKeeperRegistrar12Mock) SetRegistrationConfig(_autoApproveConfigType uint8, _autoApproveMaxAllowed uint32, _approvedCount uint32, _keeperRegistry common.Address, _minPLIJuels *big.Int) error {
	opts, err := f.client.TransactionOpts(f.client.GetDefaultWallet())
	if err != nil {
		return err
	}
	tx, err := f.registrarMock.SetRegistrationConfig(opts, _autoApproveConfigType, _autoApproveMaxAllowed, _approvedCount, _keeperRegistry, _minPLIJuels)
	if err != nil {
		return err
	}
	return f.client.ProcessTransaction(tx)
...
// Fund the PLI Token contract with ETH to distribute the token
...
func (l *EthereumLinkToken) TransferAndCall(to string, amount *big.Int, data []byte) (*types.Transaction, error) {
	...
	l.l.Info().
		...
		Msg("Transferring and Calling PLI")
...
func (o *EthereumOffchainAggregator) SetConfig(
	...
	// Gather necessary addresses and keys from our plugin nodes to properly configure the OCR contract
	log.Info().Str("Contract Address", o.address.Hex()).Msg("Configuring OCR Contract")
	for i, node := range pluginNodes {
...
// EthereumMockETHPLIFeed represents mocked ETH/PLI feed contract
type EthereumMockETHPLIFeed struct {
	client  blockchain.EVMClient
	feed    *mock_ethlink_aggregator_wrapper.MockETHPLIAggregator
	address *common.Address
}

func (v *EthereumMockETHPLIFeed) Address() string {
	return v.address.Hex()
}

func (v *EthereumMockETHPLIFeed) LatestRoundData() (*big.Int, error) {
...
func (v *EthereumMockETHPLIFeed) LatestRoundDataUpdatedAt() (*big.Int, error) {
	data, err := v.feed.LatestRoundData(&bind.CallOpts{
		From:    common.HexToAddress(v.client.GetDefaultWallet().Address()),
		Context: context.Background(),
	})
