...
type FluxAggregatorOptions struct {
	PaymentAmount *big.Int       // The amount of PLI paid to each oracle per submission, in wei (units of 10⁻¹⁸ PLI)
...
type OffchainOptions struct {
	...
	MicroLinkPerEth           uint32         // The reimbursement per ETH of gas cost, in 1e-6PLI units
	LinkGweiPerObservation    uint32         // The reward to the oracle for contributing an observation to a successfully transmitted report, in 1e-9PLI units
	LinkGweiPerTransmission   uint32         // The reward to the transmitter of a successful report, in 1e-9PLI units
...
// https://uploads-ssl.webflow.com/5f6b7190899f41fb70882d08/603651a1101106649eef6a53_plugin-ocr-protocol-paper-02-24-20.pdf
...
type KeeperRegistrar12Mock interface {
	...
	SetRegistrationConfig(_autoApproveConfigType uint8, _autoApproveMaxAllowed uint32, _approvedCount uint32, _keeperRegistry common.Address, _minPLIJuels *big.Int) error
}
