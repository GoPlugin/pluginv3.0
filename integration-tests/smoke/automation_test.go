...
import (
	...
	cltypes "github.com/goplugin/pluginv3.0/v2/core/chains/evm/types"
...
func TestMain(m *testing.M) {
	...
	// fmt.Printf("Plugin Image %v\n", config.PluginImage.Image)                                         // Print to get around disabled logging
	// fmt.Printf("Plugin Version %v\n", config.PluginImage.Version)                                     // Print to get around disabled logging
	os.Exit(m.Run())
}
...
func SetupAutomationBasic(t *testing.T, nodeUpgrade bool, automationTestConfig types.AutomationTestConfig) {
	...
	for name, registryVersion := range registryVersions {
		...
		t.Run(name, func(t *testing.T) {
			...
			var err error
			if nodeUpgrade {
				if cfg.GetPluginUpgradeImageConfig() == nil {
					t.Fatal("[PluginUpgradeImage] must be set in TOML config to upgrade nodes")
				}
			}
			...
			if nodeUpgrade {
				require.NotNil(t, cfg.GetPluginImageConfig(), "unable to upgrade node version, [PluginUpgradeImage] was not set, must both a new image or a new version")
				expect := 5
				// Upgrade the nodes one at a time and check that the upkeeps are still being performed
				for i := 0; i < 5; i++ {
					err = actions.UpgradePluginNodeVersionsLocal(*cfg.GetPluginImageConfig().Image, *cfg.GetPluginImageConfig().Version, a.DockerEnv.ClCluster.Nodes[i])
...
func TestAutomationKeeperNodesDown(t *testing.T) {
	...
	for name, registryVersion := range registryVersions {
		...
		t.Run(name, func(t *testing.T) {
			...
			gom := gomega.NewGomegaWithT(t)
			nodesWithoutBootstrap := a.PluginNodes[1:]
			...
			require.NoError(t, err, "Error deleting job from Plugin node")
			err = a.ChainClient.WaitForEvents()
...
func setupAutomationTestDocker(
	...
	l.Debug().Msgf("Funding amount: %f", *automationTestConfig.GetCommonConfig().PluginNodeFunding)
	clNodesCount := 5
	if isMercuryV02 || isMercuryV03 {
		env, err = test_env.NewCLTestEnvBuilder().
			...
			WithGeth().
			WithMockAdapter().
			WithFunding(big.NewFloat(*automationTestConfig.GetCommonConfig().PluginNodeFunding)).
			WithStandardCleanup().
			Build()
		...
		err = env.FundPluginNodes(big.NewFloat(*automationTestConfig.GetCommonConfig().PluginNodeFunding))
