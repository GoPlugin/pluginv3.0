...
import (
	...
	"github.com/goplugin/pluginv3.0/integration-tests/actions/automationv2"
	"github.com/goplugin/pluginv3.0/integration-tests/client"
	"github.com/goplugin/pluginv3.0/integration-tests/contracts"
	contractseth "github.com/goplugin/pluginv3.0/integration-tests/contracts/ethereum"
	tc "github.com/goplugin/pluginv3.0/integration-tests/testconfig"
	aconfig "github.com/goplugin/pluginv3.0/integration-tests/testconfig/automation"
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/log_emitter"
...
func TestLogTrigger(t *testing.T) {
	...
	version := *loadedTestConfig.PluginImage.Version
	image := *loadedTestConfig.PluginImage.Image
	...
	l.Info().
		...
		Str("Log Level", *loadedTestConfig.Automation.General.PluginNodeLogLevel).
		Str("Image", image).
		Str("Tag", version).
		Msg("Test Config")
...
%s`
	...
	testConfig := fmt.Sprintf(testConfigFormat, *loadedTestConfig.Automation.General.NumberOfNodes, *loadedTestConfig.Automation.General.Duration,
		*loadedTestConfig.Automation.General.BlockTime, *loadedTestConfig.Automation.General.SpecType, *loadedTestConfig.Automation.General.PluginNodeLogLevel, image, version, string(prettyLoadConfig))
	...
	automationDefaultLinkFunds := big.NewInt(0).Mul(big.NewInt(1e18), big.NewInt(int64(10000))) //10000 PLI

	registrySettings := &contracts.KeeperRegistrySettings{
		PaymentPremiumPPB:    uint32(0),
		FlatFeeMicroPLI:     uint32(40_000),
	...
	for i := 0; i < numberOfUpkeeps+1; i++ { // +1 for the OCR boot node
		var nodeTOML string
		if i == 1 || i == 3 {
			nodeTOML = fmt.Sprintf("%s\n\n[Log]\nLevel = \"%s\"", baseTOML, *loadedTestConfig.Automation.General.PluginNodeLogLevel)
		} else {
			nodeTOML = fmt.Sprintf("%s\n\n[Log]\nLevel = \"info\"", baseTOML)
		}
		...
		var overrideFn = func(_ interface{}, target interface{}) {
			ctfconfig.MustConfigOverridePluginVersion(loadedTestConfig.PluginImage, target)
			ctfconfig.MightConfigOverridePyroscopeKey(loadedTestConfig.Pyroscope, target)
		}

		cd := plugin.NewWithOverride(i, map[string]any{
			"toml":       nodeTOML,
			"plugin":  nodeSpec,
			"db":         dbSpec,
			"prometheus": *loadedTestConfig.Automation.General.UsePrometheus,
		}, loadedTestConfig.PluginImage, overrideFn)

		testEnvironment.AddHelm(cd)
	}

	err = testEnvironment.Run()
	require.NoError(t, err, "Error running plugin DON")
	...
	a := automationv2.NewAutomationTestK8s(chainClient, contractDeployer, pluginNodes)
	...
	err = actions.FundPluginNodesAddress(pluginNodes[1:], chainClient, big.NewFloat(*loadedTestConfig.Common.PluginNodeFunding), 0)
...
Test Duration: %s`
	...
	t.Cleanup(func() {
		if err = actions.TeardownRemoteSuite(t, testEnvironment.Cfg.Namespace, pluginNodes, nil, &loadedTestConfig, chainClient); err != nil {
