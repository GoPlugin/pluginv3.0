...
import (
	...
	"github.com/goplugin/plugin-common/pkg/utils/bytes"
)
...
// Link contains a field to represent the smallest units of PLI
type Link big.Int

// NewLinkFromJuels returns a new struct to represent PLI from it's smallest unit
...
// Link returns Link formatted as a string, in PLI units
...
func (l *Link) MarshalText() ([]byte, error) {
	if l.Cmp(linkFmtThreshold) >= 0 {
		return []byte(fmt.Sprintf("%s pli", decimal.NewFromBigInt(l.ToInt(), -18))), nil
	}
...
func (l *Link) UnmarshalText(text []byte) error {
	s := string(text)
	if strings.HasSuffix(s, "pli") {
		s = strings.TrimSuffix(s, "pli")
		s = strings.TrimSuffix(s, " ")
		d, err := decimal.NewFromString(s)
		if err != nil {
			return fmt.Errorf("assets: cannot unmarshal %q into a *assets.Pli: %w", text, err)
		}
	...
	if _, ok := l.SetString(s, 10); !ok {
		return fmt.Errorf("assets: cannot unmarshal %q into a *assets.Pli", text)
	}
	return nil
}
...
func (l *Link) Scan(value interface{}) error {
	...
	case string:
		...
		if !ok {
			return fmt.Errorf("unable to set string %v of %T to base 10 big.Int for Pli", value, value)
		}
		*l = *decoded
	case []uint8:
