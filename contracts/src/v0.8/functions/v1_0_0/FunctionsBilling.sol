...
abstract contract FunctionsBilling is Routable, IFunctionsBilling {
  ...
  /// @notice Gets the Plugin Coordinator's billing configuration
  /// @return config
  function getConfig() external view returns (Config memory) {
    return s_config;
  }

  /// @notice Sets the Plugin Coordinator's billing configuration
  ...
  function _getJuelsPerGas(uint256 gasPriceWei) private view returns (uint96) {
    // (1e18 juels/pli) * (wei/gas) / (wei/pli) = juels per gas
    // There are only 1e9*1e18 = 1e27 juels in existence, should not exceed uint96 (2^96 ~ 7e28)
    return SafeCast.toUint96((1e18 * gasPriceWei) / getWeiPerUnitPli());
  }
  ...
  /// @notice Estimate the cost in Juels of PLI
  ...
  /// @param request - Plugin Functions request data, see FunctionsResponse.RequestMeta for the structure
