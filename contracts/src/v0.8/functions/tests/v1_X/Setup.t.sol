...
/// @notice Set up to deploy the following contracts: FunctionsRouter, FunctionsCoordinator, PLI/ETH Feed, ToS Allow List, and PLI token
contract FunctionsRouterSetup is BaseTest {
  ...
  uint72 s_subscriptionDepositJuels = 11 * JUELS_PER_PLI;

  int256 internal PLI_ETH_RATE = 6_000_000_000_000_000;
  uint8 internal PLI_ETH_DECIMALS = 18;
  int256 internal PLI_USD_RATE = 1_500_000_000;
  uint8 internal PLI_USD_DECIMALS = 8;
...
contract FunctionsSubscriptionSetup is FunctionsClientSetup {
  ...
  uint96 s_subscriptionInitialFunding = 10 * JUELS_PER_PLI; // 10 PLI
...
contract FunctionsClientRequestSetup is FunctionsSubscriptionSetup {
  ...
  function _getExpectedCostEstimate(uint256 callbackGas) internal view returns (uint96) {
    ...
    uint96 juelsPerGas = uint96((1e18 * gasPriceWithOverestimation) / uint256(PLI_ETH_RATE));
  ...
  function _getExpectedCost(uint256 gasUsed) internal view returns (uint96) {
    uint96 juelsPerGas = uint96((1e18 * TX_GASPRICE_START) / uint256(PLI_ETH_RATE));
    uint96 gasOverheadJuels = juelsPerGas *
      (getCoordinatorConfig().gasOverheadBeforeCallback + getCoordinatorConfig().gasOverheadAfterCallback);
