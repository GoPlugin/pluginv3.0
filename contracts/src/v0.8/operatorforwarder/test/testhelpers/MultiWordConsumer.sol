...
contract MultiWordConsumer is PluginClient {
  ...
  function requestEthereumPrice(string memory, uint256 _payment) public {
    Plugin.Request memory req = _buildOperatorRequest(s_specId, this.fulfillBytes.selector);
    _sendOperatorRequest(req, _payment);
  }

  function requestMultipleParameters(string memory, uint256 _payment) public {
    Plugin.Request memory req = _buildOperatorRequest(s_specId, this.fulfillMultipleParameters.selector);
    _sendOperatorRequest(req, _payment);
  }
  ...
  function requestMultipleParametersWithCustomURLs(
    ...
    Plugin.Request memory req = _buildOperatorRequest(
      s_specId,
  ...
  function fulfillBytes(bytes32 _requestId, bytes memory _price) public recordPluginFulfillment(_requestId) {
