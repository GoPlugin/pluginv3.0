...
import {PluginClient, Plugin} from "../../../PluginClient.sol";
...
contract Plugined is PluginClient {
  ...
  function newRequest(
    ...
    return _buildPluginRequest(_specId, _callbackAddress, _callbackFunctionSignature);
  }
   ...
   */
  function pluginRequest(Plugin.Request memory _req, uint256 _payment) internal returns (bytes32) {
    return _sendPluginRequest(_req, _payment);
  }
   ...
   */
  function pluginRequestTo(
    address _oracle,
    Plugin.Request memory _req,
    uint256 _payment
  ) internal returns (bytes32 requestId) {
    return _sendPluginRequestTo(_oracle, _req, _payment);
  }
   ...
   */
  function pluginToken() internal view returns (address) {
   ...
   */
  function fulfillPluginRequest(
    bytes32 _requestId
  )
    internal
    recordPluginFulfillment(_requestId) // solhint-disable-next-line no-empty-blocks
  {}
   ...
   */
  function setPluginWithENS(address _ens, bytes32 _node) internal {
    _usePluginWithENS(_ens, _node);
  }
