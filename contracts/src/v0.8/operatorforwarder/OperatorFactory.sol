...
contract OperatorFactory {
  ...
  function deployNewOperator() external returns (address) {
    Operator operator = new Operator(pliToken, msg.sender);
  ...
  function deployNewOperatorAndForwarder() external returns (address, address) {
    ...
    AuthorizedForwarder forwarder = new AuthorizedForwarder(pliToken, address(this), address(operator), new bytes(0));
  ...
  function deployNewForwarder() external returns (address) {
    AuthorizedForwarder forwarder = new AuthorizedForwarder(pliToken, msg.sender, address(0), new bytes(0));
  ...
  function deployNewForwarderAndTransferOwnership(address to, bytes calldata message) external returns (address) {
    AuthorizedForwarder forwarder = new AuthorizedForwarder(pliToken, msg.sender, to, message);
