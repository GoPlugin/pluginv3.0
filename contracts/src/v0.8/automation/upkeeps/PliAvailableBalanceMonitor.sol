...
interface IPliAvailable {
  function pliAvailableForPayment() external view returns (int256 availableBalance);
}

/// @title The PliAvailableBalanceMonitor contract.
/// @notice A keeper-compatible contract that monitors target contracts for balance from a custom
/// function pliAvailableForPayment() and funds them with PLI if it falls below a defined
...
contract PliAvailableBalanceMonitor is AccessControl, AutomationCompatibleInterface, Pausable {
  ...
  error InvalidPliTokenAddress(address lt);
  ...
  IERC20 private immutable i_pliToken;
  ...
  function topUp(address[] memory targetAddresses) public whenNotPaused nonReentrant {
    MonitoredAddress memory contractToFund;
    uint256 minWaitPeriod = s_minWaitPeriodSeconds;
    uint256 localBalance = i_pliToken.balanceOf(address(this));
    for (uint256 idx = 0; idx < targetAddresses.length; idx++) {
      ...
      if (localBalance >= contractToFund.topUpAmount && fundingNeeded) {
        bool success = i_pliToken.transfer(target, contractToFund.topUpAmount);
        if (success) {
  ...
  function _needsFunding(
    ...
    try proxy.aggregator() returns (address aggregatorAddress) {
      ...
      target = IPliAvailable(aggregatorAddress);
    } catch {
      target = IPliAvailable(targetAddress);
    }
    try target.pliAvailableForPayment() returns (int256 balance) {
  ...
  function checkUpkeep(
    ...
    if (i_pliToken.balanceOf(address(this)) >= total_batch_balance) {
  ...
  /// @notice Withdraws the contract balance in the PLI token.
  /// @param amount the amount of the PLI to withdraw
  ...
  function withdraw(uint256 amount, address payable payee) external onlyAdminOrExecutor {
    ...
    i_pliToken.transfer(payee, amount);
