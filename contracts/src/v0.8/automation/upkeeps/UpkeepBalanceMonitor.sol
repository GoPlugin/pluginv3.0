...
/// @notice A keeper-compatible contract that monitors and funds Plugin Automation upkeeps.
contract UpkeepBalanceMonitor is ConfirmedOwner, Pausable {
  ...
  /// @member maxTopUpAmount is the maximum amount of PLI to top-up an upkeep with
  struct Config {
    uint8 maxBatchSize;
  ...
  function getUnderfundedUpkeeps() public view returns (uint256[] memory, address[] memory, uint96[] memory) {
    ...
    uint256 availableFunds = PLI_TOKEN.balanceOf(address(this));
    uint256 count;
  ...
  function topUp(
    ...
    for (uint256 i = 0; i < upkeepIDs.length; i++) {
      try PLI_TOKEN.transferAndCall(registryAddresses[i], topUpAmounts[i], abi.encode(upkeepIDs[i])) returns (
        bool success
      ) {
        if (success) {
  ...
  function withdraw(uint256 amount, address payee) external onlyOwner {
    ...
    PLI_TOKEN.transfer(payee, amount);
