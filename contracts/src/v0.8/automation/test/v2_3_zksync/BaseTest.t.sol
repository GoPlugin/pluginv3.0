...
contract BaseTest is Test {
  ...
  WETH9 internal weth;
  MockV3Aggregator internal PLI_USD_FEED;
  ...
  function setUp() public virtual {
    ...
    PLI_USD_FEED = new MockV3Aggregator(8, 2_000_000_000); // $20
  ...
  function deployAndConfigureZKSyncRegistryAndRegistrar(
    ...
    minRegistrationFees[2] = 5e18; // 5 PLI
    ...
    billingTokenConfigs[2] = AutomationRegistryBase2_3.BillingConfig({
      ...
      flatFeeMilliCents: DEFAULT_FLAT_FEE_MILLI_CENTS, // 2 cents
      priceFeed: address(PLI_USD_FEED),
    ...
    if (payoutMode == ZKSyncAutoBase.PayoutMode.OFF_CHAIN) {
      // remove PLI as a payment method if we are settling offchain
      assembly {
