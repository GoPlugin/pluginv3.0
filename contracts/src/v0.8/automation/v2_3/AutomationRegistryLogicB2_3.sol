...
contract AutomationRegistryLogicB2_3 is AutomationRegistryBase2_3, Chainable {
   ...
   * @dev this function may be deprecated in a future version of plugin automation
   */
  function checkUpkeep(
    uint256 id
  )
  ...
  /**
   * @notice withdraws excess PLI from the liquidity pool
  ...
  /**
   * @notice withdraws non-PLI fees earned by the contract
   * @param asset the asset to withdraw
   * @param to the address to send the fees to
   * @param amount the amount to withdraw
   * @dev in ON_CHAIN mode, we prevent withdrawing non-PLI fees unless there is sufficient PLI liquidity
