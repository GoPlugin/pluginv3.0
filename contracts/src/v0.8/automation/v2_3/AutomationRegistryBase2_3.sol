...
abstract contract AutomationRegistryBase2_3 is ConfirmedOwner {
   ...
   * @member balance a node's balance in PLI
   * @member lastCollected the total balance at which the node last withdrew
   * @dev uint96 is safe for balance / last collected because transmitters are only ever paid in PLI
   */
  struct Transmitter {
    bool active;
    uint8 index;
  ...
  function _handlePayment(
    ...
    // scaling factors to adjust decimals between billing token and PLI
    uint256 decimals = paymentParams.billingTokenParams.decimals;
