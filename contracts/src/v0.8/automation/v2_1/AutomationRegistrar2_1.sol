...
contract AutomationRegistrar2_1 is TypeAndVersionInterface, ConfirmedOwner, IERC677Receiver {
  ...
  event ConfigChanged(address keeperRegistry, uint96 minPLIJuels);
   ...
   * @param minPLIJuels minimum PLI that new registrations should fund their upkeep with
   */
  function setConfig(address keeperRegistry, uint96 minPLIJuels) public onlyOwner {
    s_config = RegistrarConfig({minPLIJuels: minPLIJuels, keeperRegistry: IKeeperRegistryMaster(keeperRegistry)});
    emit ConfigChanged(keeperRegistry, minPLIJuels);
  }
   ...
   */
  function getConfig() external view returns (address keeperRegistry, uint256 minPLIJuels) {
    RegistrarConfig memory config = s_config;
    return (address(config.keeperRegistry), config.minPLIJuels);
  }
