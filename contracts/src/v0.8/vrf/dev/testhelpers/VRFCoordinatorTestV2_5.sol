...
contract VRFCoordinatorTestV2_5 is VRFOld, SubscriptionAPI, IVRFCoordinatorV2Plus {
   ...
   * should be made in native or PLI. Payment in PLI is only available if the PLI token is available to this contract.
  ...
  function _getRandomnessFromProof(
    ...
    uint256 randomness = VRFOld._randomValueFromVRFProof(proof, actualSeed); // Reverts on failure
  ...
  function migrate(uint256 subId, address newCoordinator) external nonReentrant {
    ...
    // Only transfer PLI if the token is active and there is a balance.
    if (address(PLI) != address(0) && balance != 0) {
      // solhint-disable-next-line gas-custom-errors
      require(PLI.transfer(address(newCoordinator), balance), "insufficient funds");
    }
