...
abstract contract SubscriptionAPI is ConfirmedOwner, IERC677Receiver, IVRFSubscriptionV2Plus {
  ...
  function recoverFunds(address to) external onlyOwner {
    ...
    // It is possible that this coordinator address was funded with PLI
  ...
  /*
   * @notice withdraw PLI earned through fulfilling requests
  ...
  function withdraw(address recipient) external nonReentrant onlyOwner {
    ...
    _requireSufficientBalance(PLI.transfer(recipient, amount));
  }
  ...
  function _cancelSubscriptionHelper(uint256 subId, address to) internal {
    ...
    // Only withdraw PLI if the token is active and there is a balance.
    if (address(PLI) != address(0) && balance != 0) {
      _requireSufficientBalance(PLI.transfer(to, uint256(balance)));
    }
