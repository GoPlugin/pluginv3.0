 ...
 * on a receiving plugin node watching for RunRequests coming from that
...
export interface RunRequest {
  ...
  requestId: string
  /**
   * The amount of PLI used for payment
   *
   * @solformat uint256
   */
  payment: string
...
/**
 * Extract a javascript representation of a ConcretePlugined#Request event
 * from an EVM log.
 * ```solidity
 *  event Request(
 *    bytes32 id,
 ...
 *  );
 * ```
 * The request event is emitted from the `ConcretePlugined.sol` testing contract.
 *
