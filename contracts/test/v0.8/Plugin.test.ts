...
const debug = makeDebug('PluginTestHelper')
let concretePluginFactory: ContractFactory

let roles: Roles

before(async () => {
  roles = (await getUsers()).roles
  concretePluginFactory = await ethers.getContractFactory(
    'src/v0.8/tests/PluginTestHelper.sol:PluginTestHelper',
    roles.defaultAccount,
  )
})

describe('PluginTestHelper', () => {
  let ccl: Contract
  let defaultAccount: Signer

  beforeEach(async () => {
    defaultAccount = roles.defaultAccount
    ccl = await concretePluginFactory.connect(defaultAccount).deploy()
  })
