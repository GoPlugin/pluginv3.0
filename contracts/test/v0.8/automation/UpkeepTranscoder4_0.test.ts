...
const setup = async () => {
  ...
  pliEthFeed = await mockV3AggregatorFactory.connect(owner).deploy(9, pliEth)
  ...
  describe('#transcodeUpkeeps', () => {
    ...
    context('when from version is correct', () => {
      ...
      it('migrates upkeeps from 1.2 registry to 2.1', async () => {
        ...
        expect(
          (await registry21.getState()).state.expectedPliBalance,
        ).to.equal(toWei('1000'))
        expect(await pliToken.balanceOf(registry21.address)).to.equal(
          toWei('1000'),
        )
