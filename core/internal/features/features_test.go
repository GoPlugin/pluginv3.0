...
import (
	...
	"github.com/goplugin/pluginv3.0/v2/core/chains/evm/forwarders"
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/consumer_wrapper"
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/multiwordconsumer_wrapper"
	...
	"github.com/goplugin/pluginv3.0/v2/core/services/keystore/keys/keystest"
...
func TestIntegration_DirectRequest(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			config := configtest.NewGeneralConfigSimulated(t, func(c *plugin.Config, s *plugin.Secrets) {
...
func setupAppForEthTx(t *testing.T, operatorContracts OperatorContracts) (app *cltest.TestApplication, sendingAddress common.Address, o *observer.ObservedLogs) {
	...
	cfg := configtest.NewGeneralConfigSimulated(t, func(c *plugin.Config, s *plugin.Secrets) {
		c.Database.Listener.FallbackPollInterval = commonconfig.MustNewDuration(100 * time.Millisecond)
	})
...
"""
`
		// This data is a call to link token's `transfer` function and will revert due to insufficient PLI on the sender address
...
func setupOCRContracts(t *testing.T) (*bind.TransactOpts, *backends.SimulatedBackend, common.Address, *offchainaggregator.OffchainAggregator, *flags_wrapper.Flags, common.Address) {
	...
	ocrContractAddress, _, ocrContract, err := offchainaggregator.DeployOffchainAggregator(owner, b,
		...
		3.6e7,            // 3.6e7 microPLI, or 36 PLI
...
func setupNode(t *testing.T, owner *bind.TransactOpts, portV2 int,
	b *backends.SimulatedBackend, overrides func(c *plugin.Config, s *plugin.Secrets),
...
func setupForwarderEnabledNode(t *testing.T, owner *bind.TransactOpts, portV2 int, b *backends.SimulatedBackend, overrides func(c *plugin.Config, s *plugin.Secrets)) (*cltest.TestApplication, string, common.Address, common.Address, ocrkey.KeyV2) {
	p2pKey := keystest.NewP2PKeyV2(t)
	config, _ := heavyweight.FullTestDBV2(t, func(c *plugin.Config, s *plugin.Secrets) {
		c.Insecure.OCRDevelopmentMode = ptr(true) // Disables ocr spec validation so we can have fast polling for the test.
	...
	for _, tt := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			for i := 0; i < numOracles; i++ {
				app, peerID, transmitter, key := setupNode(t, owner, ports[i], b, func(c *plugin.Config, s *plugin.Secrets) {
...
func TestIntegration_OCR_ForwarderFlow(t *testing.T) {
	...
	t.Run("ocr_forwarder_flow", func(t *testing.T) {
		...
		ports := freeport.GetN(t, numOracles)
		for i := 0; i < numOracles; i++ {
			app, peerID, transmitter, forwarder, key := setupForwarderEnabledNode(t, owner, ports[i], b, func(c *plugin.Config, s *plugin.Secrets) {
