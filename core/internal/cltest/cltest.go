...
import (
	...
	"github.com/goplugin/pluginv3.0/v2/common/client"
	commonmocks "github.com/goplugin/pluginv3.0/v2/common/types/mocks"
	...
	"github.com/goplugin/pluginv3.0/v2/core/services/keystore/keys/ocrkey"
	...
	clsessions "github.com/goplugin/pluginv3.0/v2/core/sessions"
	...
	webauth "github.com/goplugin/pluginv3.0/v2/core/web/auth"
	webpresenters "github.com/goplugin/pluginv3.0/v2/core/web/presenters"
	"github.com/goplugin/pluginv3.0/v2/plugins"

	// Force import of pgtest to ensure that txdb is registered as a DB driver
	_ "github.com/goplugin/pluginv3.0/v2/core/internal/testutils/pgtest"
)

const (
	// Collection of test fixture DB user emails per role
	APIEmailAdmin    = "apiuser@plugin.test"
	APIEmailEdit     = "apiuser-edit@plugin.test"
	APIEmailRun      = "apiuser-run@plugin.test"
	APIEmailViewOnly = "apiuser-view-only@plugin.test"
...
func NewApplicationWithConfigAndKey(t testing.TB, c plugin.GeneralConfig, flagsAndDeps ...interface{}) *TestApplication {
...
func NewApplicationWithConfig(t testing.TB, cfg plugin.GeneralConfig, flagsAndDeps ...interface{}) *TestApplication {
	t.Helper()
	...
	evmOpts := plugin.EVMFactoryConfig{
	...
	initOps := []plugin.CoreRelayerChainInitFunc{plugin.InitEVM(testCtx, relayerFactory, evmOpts)}
	...
	if cfg.CosmosEnabled() {
		...
		initOps = append(initOps, plugin.InitCosmos(testCtx, relayerFactory, cosmosCfg))
	}
	if cfg.SolanaEnabled() {
		solanaCfg := plugin.SolanaFactoryConfig{
			Keystore:    keyStore.Solana(),
			TOMLConfigs: cfg.SolanaConfigs(),
		}
		initOps = append(initOps, plugin.InitSolana(testCtx, relayerFactory, solanaCfg))
	}
	if cfg.StarkNetEnabled() {
		starkCfg := plugin.StarkNetFactoryConfig{
			Keystore:    keyStore.StarkNet(),
			TOMLConfigs: cfg.StarknetConfigs(),
		}
		initOps = append(initOps, plugin.InitStarknet(testCtx, relayerFactory, starkCfg))
	...
	appInstance, err := plugin.NewApplication(plugin.ApplicationOpts{
		Config:                     cfg,
		MailMon:                    mailMon,
		SqlxDB:                     db,
		KeyStore:                   keyStore,
	...
	require.NoError(t, err)
	app := appInstance.(*plugin.PluginApplication)
	ta := &TestApplication{
		t:                    t,
...
// Start starts the plugin app and registers Stop to clean up at end of test.
func (ta *TestApplication) Start(ctx context.Context) error {
	ta.t.Helper()
	ta.Started = true
	err := ta.PluginApplication.KeyStore.Unlock(Password)
	if err != nil {
		return err
	}

	err = ta.PluginApplication.Start(ctx)
	if err != nil {
		return err
	}
...
func (ta *TestApplication) Stop() error {
	...
	err := ta.PluginApplication.StopIfStarted()
	if ta.Server != nil {
		ta.Server.Close()
	}
	return err
...
func (ta *TestApplication) NewHTTPClient(user *User) HTTPClientCleaner {
	...
	if user.Email == "" {
		user.Email = fmt.Sprintf("%s@plugin.test", uuid.New())
	}
...
func (ta *TestApplication) NewShellAndRenderer() (*cmd.Shell, *RendererMock) {
	...
	client := &cmd.Shell{
		...
		Config:                         ta.GetConfig(),
		Logger:                         lggr,
		AppFactory:                     seededAppFactory{ta.PluginApplication},
...
// MockApplicationEthCalls mocks all calls made by the plugin application as
