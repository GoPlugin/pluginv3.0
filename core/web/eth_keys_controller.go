...
func NewETHKeysController(app plugin.Application) *ETHKeysController {
...
// Index returns the node's Ethereum keys and the account balances of ETH & PLI.
// Example:
//
...
// queries the EthClient for the PLI balance at the address associated with state
func (ekc *ETHKeysController) getLinkBalance(ctx context.Context, state ethkey.State) *commonassets.Link {
	...
	if err != nil {
		...
		if err != nil {
			ekc.lggr.Errorw("Failed to get PLI balance", "chainID", chainID, "address", state.Address, "err", err)
		}
	}
	return bal
}
