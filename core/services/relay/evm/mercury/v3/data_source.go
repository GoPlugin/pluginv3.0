...
import (
	...
	v3types "github.com/goplugin/plugin-common/pkg/types/mercury/v3"
	v3 "github.com/goplugin/plugin-data-streams/mercury/v3"
	...
	"github.com/goplugin/pluginv3.0/v2/core/services/relay/evm/mercury/types"
	mercurytypes "github.com/goplugin/pluginv3.0/v2/core/services/relay/evm/mercury/types"
	mercuryutils "github.com/goplugin/pluginv3.0/v2/core/services/relay/evm/mercury/utils"
	"github.com/goplugin/pluginv3.0/v2/core/services/relay/evm/mercury/v3/reportcodec"
...
func (ds *datasource) Observe(ctx context.Context, repts ocrtypes.ReportTimestamp, fetchMaxFinalizedTimestamp bool) (obs v3types.Observation, pipelineExecutionErr error) {
	...
	if ds.feedID == ds.linkFeedID {
		...
		go func() {
			...
			if obs.LinkPrice.Val == nil && obs.LinkPrice.Err == nil {
				...
				ds.lggr.Warnw(fmt.Sprintf("Mercury server was missing PLI feed, using sentinel value of %s", v3.MissingPrice), "linkFeedID", ds.linkFeedID)
				...
				ds.lggr.Errorw("Mercury server returned error querying PLI price feed", "err", obs.LinkPrice.Err, "linkFeedID", ds.linkFeedID)
			}
		}()
	}
	...
	if isLink || isNative {
		...
		if isLink {
			// This IS the PLI feed, use our observed price
